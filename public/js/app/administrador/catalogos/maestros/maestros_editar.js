...
addFirst();
function addFirst(){
    var temporal = JSON.parse(document.getElementById("grupos").value);
    for(var i = 0; i < temporal.length; i++){
        gruposArray.push(temporal[i].grupo_id);
    }
    materiasFind();
    temporal = JSON.parse(document.getElementById("materias").value);
    for(var i = 0; i < temporal.length; i++){
        materiasArray.push(temporal[i].materia_id);
    }
